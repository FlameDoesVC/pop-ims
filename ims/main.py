...
class UserDTO:
    ...
    def edit():
        ...
        if answers is None:
            return
        user = orm.select(
            user for user in User if user.username == answers["user"]
        ).first()
        ...
        answers = inquirer.prompt(
            [
                ...
                inquirer.Password(
                    "password",
                    message="Enter the new password (leave blank for same as old password)",
                    default="",
                ),
                inquirer.Password(
                    "password2",
                    message="Re-enter the new password",
                    default="",
                    ignore=lambda answers: answers["password"] == "",
                    validate=validate_password,
                ),
                inquirer.Checkbox(
                    "role",
...
def login_menu(first_login=False):
    ...
    menu = [
        inquirer.List(
            "option",
            message="Users",
            choices=["Login", "Register", "Edit", "Delete", "Back", "Exit"],
            hints={
