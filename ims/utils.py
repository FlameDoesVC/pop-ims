	...
	def validator(answers, value):
		...
		if type == "int":
			try:
				...
				raise errors.ValidationError('', reason=f"{value_name} must be a valid integer")
		elif type == "float":
			try:
				value = float(value)
			except Exception:
				raise errors.ValidationError('', reason=f"{value_name} must be a valid float")
		elif type == "str":
			if not isinstance(value, str):
				raise errors.ValidationError('', reason=f"{value_name} must be a valid string")
			if len(value) == 0:
				raise errors.ValidationError('', reason=f"{value_name} must not be empty")
		return True
	return validator
