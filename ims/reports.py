...
class ReportGenerator:
    ...
    def inventory_report():
        ...
        print(text)
        markdown = inquirer.confirm(
            "Do you want to save the report as a markdown file?"
        )
        if not markdown:
            return
        ...
        file = inquirer.prompt(
            [
                inquirer.Text(
                    "file",
                    message="Enter the file name to save the report as",
                    default=f"inventory_report.md",
                )
            ]
        )
        if not file:
    ...
    def sales_report():
        ...
        for order in last_30_days:
            for order_item in order.items:
                if order_item.item.category not in [
                    item["category"] for item in items_sold_last_30_days
                ]:
                    items_sold_last_30_days.append(
                        {
        ...
        for order in last_year:
            for order_item in order.items:
                if order_item.item.category not in [
                    item["category"] for item in items_sold_last_year
                ]:
                    items_sold_last_year.append(
                        {
                            "category": order_item.item.category,
                            "quantity": order_item.quantity,
                        }
